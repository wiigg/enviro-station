...
print("""combined-iot-hub.py - Combines sending data to Azure IoT Hub and displays data on device screen
================================================================================================
Reads temperature, pressure, humidity,
PM2.5, PM10, PM1 from Enviro plus and sends data to Azure IoT Hub.
...
# Read values from BME280 and PMS5003 and return as str
def read_values(curtime, comp_temp, mod_press, raw_humid, raw_pm25, raw_pm10, raw_pm1):
    values = {}
    values["curtime"] = str(curtime)
    values["temperature"] = "{:.2f}".format(comp_temp)
    values["pressure"] = "{:.2f}".format(mod_press)
    values["humidity"] = "{:.2f}".format(raw_humid)
    values["P25"] = str(raw_pm25)
    values["P10"] = str(raw_pm10)
    values["P1"] = str(raw_pm1)
...
# Main loop to read data, display, and send to Azure IoT Hub
while True:
    try:
        curtime = time.time()
        ...
        try:
            ...
            raw_pm1 = pm_values.pm_ug_per_m3(1.0)
        ...
        if time_since_update > 10:
            values = read_values(curtime, comp_temp, raw_press*100,
                                 raw_humid, raw_pm25, raw_pm10, raw_pm1)
            update_time = curtime
        ...
        # Now comes the display functionality:
        ...
        if mode == 7:
            # variable = "pm1"
            unit = "ug/m3"
            display_text(variables[mode], float(raw_pm1), unit)
